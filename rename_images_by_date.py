...
IMG_EXTENSIONS = {'.jpg', '.jpeg', '.png', '.gif', '.bmp', '.webp', '.tiff', '.tif', '.heic', '.heif'}
...
def get_new_filename(image_path, counter=1):
...
def select_files_by_pattern(image_files):
    print("\n" + "=" * 50)
    print("  文件选择模式")
    print("=" * 50)
    print("1. 全部选择")
    ...
    if choice == '3':
        ...
        return [f for f in image_files if keyword.lower() in f.name.lower()]

    if choice == '4':
        names_input = input("请输入文件名（逗号分隔）: ").strip()
        name_list = [n.strip() for n in names_input.split(',')]
        return [f for f in image_files if f.name in name_list or any(n in f.name for n in name_list)]
...
def rename_images(folder_path, dry_run=False):
    ...
    if not folder.exists():
        ...
        print("提示：")
        print("  1. 确保路径拼写正确")
        print("  2. 使用绝对路径，如: D:\\photos\\vacation")
        print("  3. 路径中不要包含引号")
    ...
    if not image_files:
        print("文件夹中没有找到图片文件")
        print(f"支持的格式: {', '.join(sorted(IMG_EXTENSIONS))}")
        return

    selected_files = select_files_by_pattern(image_files)
    if not selected_files:
        print("没有选中任何文件")
        return

    skipped_count = len(image_files) - len(selected_files)
    ...
    for image_path in selected_files:
        if image_path.name.startswith('20') and len(image_path.name) >= 8:
            skipped_count += 1
            continue

        counter_key = image_path.stem.rsplit('_', 1)[0] if '_' in image_path.stem else image_path.stem
        counter_dict[counter_key] = counter_dict.get(counter_key, 0) + 1

        new_name = get_new_filename(image_path, counter_dict[counter_key])
...
if __name__ == '__main__':
    ...
    print("=" * 50)
    print("\n使用说明:")
    print("  - 输入文件夹路径，如: D:\\photos\\vacation")
    print("  - 路径不需要加引号")
    print("  - 支持相对路径和绝对路径")
    print()
    ...
    rename_images(folder, dry_run=dry_run)